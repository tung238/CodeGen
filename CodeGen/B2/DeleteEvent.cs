...
{
    public class DeleteEvent : ICodeGenCommand
    {
        private readonly DomainModel _classDef;
        private CompilationUnitSyntax _compilationUnit;

        public DeleteEvent(DomainModel classDef)
        {
        ...
        {
            return $"{_classDef.Name}Deleted.cs";
        }

        public string GetFilePath()
        {
            return System.IO.Path.Combine(System.IO.Path.GetDirectoryName(_classDef.FilePath), "Events", GetFileName());
        }
    ...
    SyntaxFactory.List<UsingDirectiveSyntax>(
        ...
        .WithMembers(
            SyntaxFactory.SingletonList<MemberDeclarationSyntax>(
                SyntaxFactory.ClassDeclaration($"{_classDef.Name}Deleted")
                .WithModifiers(
                    ...
                    SyntaxFactory.BaseList(
                        SyntaxFactory.SingletonSeparatedList<BaseTypeSyntax>(
                            SyntaxFactory.SimpleBaseType(
                                SyntaxFactory.IdentifierName("IEvent")))))
                .WithMembers(
        ...
        private IEnumerable<MemberDeclarationSyntax> GetMember()
        {
            ...
            #region Properties
            foreach (var p in _classDef.Properties.Where(p=>p.Name.Equals("Id", StringComparison.InvariantCultureIgnoreCase)))
            {
            ...
            result.Add(SyntaxFactory.ConstructorDeclaration(
                                SyntaxFactory.Identifier($"{_classDef.Name}Deleted"))
                            .WithModifiers(
                                SyntaxFactory.TokenList(
            ...
            constructorBody.Add(SyntaxFactory.ReturnStatement(
                        SyntaxFactory.ObjectCreationExpression(
                            SyntaxFactory.IdentifierName($"{_classDef.Name}Deleted"))
                        .WithArgumentList(
                            SyntaxFactory.ArgumentList(
